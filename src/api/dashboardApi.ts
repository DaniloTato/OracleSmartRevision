...
export async function getKpiSummary(projectId: number, sprintId: number) {
    const res = await fetch(
        `/api/projects/${projectId}/kpis/summary?sprintId=${sprintId}`
    )
    return res.json()
}

export async function getTasksByUserAndSprint(
    projectId: number,
    sprintId: number
) {
    const res = await fetch(
...
export async function getTasksByUser(projectId: number) {
    const res = await fetch(`/api/projects/${projectId}/kpis/tasks-by-user`)
    return res.json()
}
...
export async function getRealHoursByUser(projectId: number, sprintId: number) {
    const res = await fetch(
        `/api/projects/${projectId}/kpis/real-hours-by-user?sprintId=${sprintId}`
    )
    return res.json()
}
