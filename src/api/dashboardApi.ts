...
export async function getTasksByUser(projectId: number, sprintId: number) {
  const res = await fetch(`/api/projects/${projectId}/kpis/tasks-by-user?sprintId=${sprintId}`)
  return res.json()
}

export async function getHoursByUser(projectId: number, sprintId: number) {
  const res = await fetch(`/api/projects/${projectId}/kpis/hours-by-user?sprintId=${sprintId}`)
  return res.json()
}
