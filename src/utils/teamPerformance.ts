...
export function buildMemberDetail(params: {
   ...
   const bonus =
      Math.abs(avgVariation) <= 2 ? 10 : Math.abs(avgVariation) <= 4 ? 5 : 0

   const scoreProductividad = Math.min(100, Math.max(0, completionRate + bonus))
   ...
   const bestType = Object.entries(completedByType).sort(
      (a, b) => b[1] - a[1]
   )[0]
